...
#difficulty select
difficulty_select_dropdown_label = label_generator('Select difficulty: ', 75)
...
def generate_button_clicked(b):
    with text_to_type_output:
        ...
        try:
            ...
            display(HTML(f"""
                <div style='background-color: black; padding-inline: 2.5%; border: 2px double white; border-radius: 2.5%; height: 275px;'>
                    <p style='color: whitesmokeS; font-weight: bold;'>⌨️ Text to type: </p>
                    <span style='font-weight: 700; color: white; '>{text_to_type}</span>
                </div>
            """))
...
gui=widgets.VBox(
    [header,gui_description,difficulty_select_dropdown,generate_button,text_to_type_output],
    layout=widgets.Layout(
        width='650px',
        height='2000px',
        margin='10px',
